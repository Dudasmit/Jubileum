...
def drawInput():
    return  html.Div([
        dbc.Card(
            dbc.CardBody(dbc.Row(CreateInput__()))
        ),  
    ])
...
def update_table(value, value_yaer):
    ...
    if value_yaer:
        ...
        #df = session.query(Anniversaries).filter_by(Anniversary=value_yaer).all()
        #df = pd.read_sql_table('Anniversaries', engine.connect())
        #df = pd.read_sql_query(sql = '''Select  * FROM  Anniversaries''' ,con = engine)
